...
위 구조는 Git에서 추적하는 공개 저장소의 주요 루트 파일과 디렉터리를 기준으로 합니다. `.github/workflows`는 CI를, `config/checkstyle`은 Java 스타일 검사를, `scripts`는 공개 저장소 위생과 정적 자산 및 줄 끝 검증을 담당합니다.

정적 스타일은 `src/main/resources/static/css/site.css`를 진입점으로 사용합니다. 실제 규칙은 공통 UI, 홈/인증, 운영 지표, 기록/라이브러리, 반응형/인쇄 스타일로 나누어 관리합니다.
...
| `identity` | Workspace ID, 이메일, Google subject, 비밀번호 값 객체 |
| `repository` | Spring Data JPA 저장소 |
| `security` | 사용자 조회, Google OAuth 성공 처리, rate limit |
...
| --- | --- | --- |
| 홈 | `/` | 제품 홈 |
| 인증 | `/login`, `/register`, `/sign-in-help`, `/forgot-password`, `/oauth2/authorization/google`, `/login/oauth2/code/google` | 로그인, 가입, 로그인 도움말, Google 계정 확인 |
| 외부 브라우저 안내 | `/external-browser-required` | 임베디드 브라우저 차단 안내 |
...
| 운영 지표 | `/admin/operations` | 관리자 전용 성장, 루틴, 목표 완료율, 내보내기, 보안 지표 |
| 오류 | `/error`, `/error/not-found` | 제품형 404 화면 |
| 상태 확인 | `/actuator/health`, `/actuator/health/**` | 런타임 상태 확인 |

`/forgot-password`는 기존 링크 호환용 경로이며 실제 화면은 `/sign-in-help`로 리다이렉트합니다.
...
- `operation_usage_event`: 로그인, 주간 리뷰 조회, 라이브러리 조회, Markdown/PDF 내보내기 이벤트
- `weekly_operation_metric_snapshot`: Growth, Routine, Goal Completion, Export, Security용 주간 운영 지표 스냅샷

`daymark_entry`는 하루 기록을 하나의 Markdown 문자열로 저장하지 않습니다. 목표, 집중 영역, 예상 변수, 저녁 체크리스트, 성과, 개선점, 감사, 메모를 섹션별 텍스트로 저장합니다.

이 구조로 다음 동작을 안정적으로 처리할 수 있습니다.
...
| `DaymarkService` | 날짜별 기록 읽기, 아침/저녁 저장, 주간 기록 상태 조회 |
| `DaymarkWeeklyProgressService` | 주간 리뷰 화면의 날짜 범위와 목표 완료율 구성 |
| `DaymarkRecordViewService` | 아침 계획 HTML, 저녁 목표 폼, PDF 출력용 기록 HTML 구성 |
| `DaymarkLibraryService` | 장기 기록 검색, 타임라인, 추세, 캘린더, Markdown 내보내기 |
| `DaymarkLibraryMarkdownExporter` | 라이브러리 검색 결과의 Markdown 내보내기 본문 구성 |
...
| `WeeklyOperationsSummaryScheduler` | 주간 운영 지표 스냅샷 저장과 운영 알림 |
| `AdministratorAccountInitializer` | 설정된 운영자 Workspace ID에 관리자 권한 부여 |

## 운영 지표 구조

관리자 화면은 단순히 보기 좋은 숫자보다 실제 운영 판단에 도움이 되는 지표를 우선합니다.
...
`Routine Days`는 전체 사용자가 실제로 작성한 사용자-날짜 수입니다. 예를 들어 3명이 각각 2일씩 작성하면 6일로 집계합니다. 운영자 계정의 활동은 사용자 추이를 왜곡하지 않도록 제외합니다.
...
주요 테스트는 실제 웹 흐름을 중심으로 구성합니다.

- Google 계정 확인 이후 Workspace 생성
...
- `AuthenticationWebFlowIntegrationTests`
- `DaymarkRecordWebFlowIntegrationTests`
- `MySqlIntegrationTests`
- `OperationsAdminWebFlowIntegrationTests`
- `ProductPageWebFlowIntegrationTests`
- `WeeklyOperationsSummaryServiceTests`
- `WeeklyOperationsSummaryTests`
- `dto/daymark/DaymarkLibraryCalendarDayDtoTests`
- `dto/daymark/DaymarkLibraryContentBlockDtoTests`
- `dto/daymark/DaymarkLibraryItemDtoTests`
- `dto/daymark/DaymarkLibraryViewDtoTests`
- `dto/daymark/WeeklyProgressItemDtoTests`
- `dto/daymark/WeeklyProgressViewDtoTests`
- `entity/UserAccountIdTests`
- `security/RateLimitCheckTests`
- `service/OperationalAlertTests`
- `support/DaymarkEntryDateTests`
- `support/DaymarkGoalCompletionCountsTests`
- `support/DaymarkGoalMarkdownTests`
- `support/DaymarkPreviewMarkdownTests`
- `support/DaymarkSectionTextTests`
- `support/DaymarkWeekOffsetTests`
