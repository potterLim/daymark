...
@ConditionalOnProperty(
    prefix = "day-log.operations.production-readiness",
    name = "enabled",
    havingValue = "true"
)
public class ProductionReadinessValidator {
    ...
    private static final String DEFAULT_MAIL_FROM_ADDRESS = "no-reply@daylog.local";
    ...
    private void validateRememberMeKey(List<String> validationErrors) {
        String rememberMeKey = mDayLogApplicationProperties.getSecurity().getRememberMeKey();
        int minimumRememberMeKeyLength =
            mDayLogApplicationProperties.getOperations().getProductionReadiness().getMinimumRememberMeKeyLength();

        if (rememberMeKey == null || rememberMeKey.isBlank()) {
            validationErrors.add("day-log.security.remember-me-key must not be blank.");
            return;
        }

        if (rememberMeKey.length() < minimumRememberMeKeyLength) {
            validationErrors.add(
                "day-log.security.remember-me-key must be at least %d characters long."
        ...
        if (
            ...
            validationErrors.add("day-log.security.remember-me-key must not use a placeholder or development secret.");
        }
    }

    private void validateSecureSessionCookie(List<String> validationErrors) {
        if (!mDayLogApplicationProperties.getOperations().getProductionReadiness().isRequireSecureSessionCookie()) {
            return;
        }
    ...
    private void validateSmtpConfiguration(List<String> validationErrors) {
        if (!mDayLogApplicationProperties.getOperations().getProductionReadiness().isRequireSmtp()) {
            return;
        }
        ...
        String mailFromAddress = mDayLogApplicationProperties.getMail().getFromAddress();
        if (mailFromAddress == null || mailFromAddress.isBlank()) {
            validationErrors.add("day-log.mail.from-address must not be blank in production.");
            return;
        }

        if (DEFAULT_MAIL_FROM_ADDRESS.equalsIgnoreCase(mailFromAddress)) {
            validationErrors.add("day-log.mail.from-address must be replaced with a real sender address in production.");
        }
    }

    private void validateAlertWebhook(List<String> validationErrors) {
        if (!mDayLogApplicationProperties.getOperations().getProductionReadiness().isRequireAlertWebhook()) {
            return;
        }

        String alertWebhookUrl = mDayLogApplicationProperties.getOperations().getAlertWebhookUrl();
        if (alertWebhookUrl == null || alertWebhookUrl.isBlank()) {
            validationErrors.add("day-log.operations.alert-webhook-url must be configured in production.");
        }
    }
}
